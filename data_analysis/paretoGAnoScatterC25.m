...
%for cluster 25 of 8 particle lattice data set
clear;
...
if (max_types >=3)
    paretoFile3 = "c25pareto3.txt";
    P = importdata(paretoFile3);
    plotData(P, co(3,:));
end

%get the 2 type pareto front data
paretoFileA = "c25pareto00110010.txt";
paretoFileB = "c25pareto01100001.txt";
paretoFileC = "c25pareto00110011.txt";
PA = importdata(paretoFileA);
PB = importdata(paretoFileB);
PC = importdata(paretoFileC);
PA = combineSets(PA,PB);
PA = combineSets(PA,PC);
PB = combineSets(PB,PA);
PB = combineSets(PB,PC);
PC = combineSets(PC,PA);
PC = combineSets(PC,PB);
...
function plotData(P,color)
    ...
    plot(EQ(1:skip:end),RATE(1:skip:end),'.', 'color',color,'markersize',20)
end
