...
if (max_types >=3)
    paretoFileA = "pareto" + state + "AABACBC.txt";
    paretoFileB = "pareto" + state + "AABCCCC.txt";
...
% paretoFileA = "pareto" + state + "ABAAABA.txt";
% paretoFileB = "pareto" + state + "AABABAA.txt";
% PA = importdata(paretoFileA);
% PB = importdata(paretoFileB);
% PA = combineSets(PA,PB);
% PB = combineSets(PB,PA);
...
if (params == 1)
    ...
    plotParameters3(PA,1);
    plotParameters3(PB,1);
    xlabel("Equilibrium Probability")
...
function plotParameters3(P,dot)
    %plot parameter data in P - 3 types
    ...
    if (jumpIndex > 0)
        EQ = [EQ(1:jumpIndex); NaN; EQ(jumpIndex+1:end)];
        w = [w(1:jumpIndex,:); NaN*ones(1,n); w(jumpIndex+1:end,:)];
    end
    
    if dot==1
        plot(EQ, w(:,4), '.','color',co(4,:), 'linewidth',3)
    else
        plot(EQ, w(:,4),'color',co(4,:), 'linewidth',3)
    end
    plot(EQ, w(:,1), 'color', co(1,:),'linewidth',3)
    plot(EQ, w(:,2), 'color',co(2,:),'linewidth',3)
    plot(EQ, w(:,3), 'color',co(3,:),'linewidth',3)
    plot(EQ, w(:,5), 'color',co(5,:),'linewidth',3)
    plot(EQ, w(:,6), 'color',co(6,:),'linewidth',3)
